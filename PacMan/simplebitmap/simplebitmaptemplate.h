...
public:
	...
	*/
	void CutOut(SimpleBitmapTemplate <ComponentType,NumComponentPerPixel> &destination,int thisX0,int thisY0,int wid,int hei,ComponentType clearColor) const
	{
		...
		int dest_no = 0;
		for(int i=thisY0;i<thisY0+hei;++i){
			for(int j=thisX0;j<thisX0+wid;++j){
				if(this->IsInRange(j, i)){
					const ComponentType* req_pixel = GetPixelPointer(j, i);
					ComponentType* dest_pixel = destination.GetEditablePixelPointer(j-thisX0, i-thisY0);
					dest_pixel[0] = req_pixel[0];
					dest_pixel[1] = req_pixel[1];
					dest_pixel[2] = req_pixel[2];
					dest_pixel[3] = req_pixel[3];
				}
			}
		}
	}
